...
impl<T: Copy> queue::Queue<T> for RingBuffer<'_, T> {
    ...
    fn is_full(&self) -> bool {
        self.head == unsafe { core::hint::assert_unchecked(self.ring.len() != 0); ((self.tail + 1) % self.ring.len()) }
    }
    ...
    fn enqueue(&mut self, val: T) -> bool {
        if self.is_full() {
            ...
            self.tail = unsafe { core::hint::assert_unchecked(self.ring.len() != 0); (self.tail + 1) % self.ring.len() };
            true
        }
    }
    ...
    fn push(&mut self, val: T) -> Option<T> {
        let result = if self.is_full() {
            ...
            self.head = unsafe { core::hint::assert_unchecked(self.ring.len() != 0); (self.head + 1) % self.ring.len() };
            Some(val)
        } else {
            None
        };
        ...
        F: Fn(&T) -> bool,
    {
        ...
        while slot != self.tail {
            if f(&self.ring[slot]) {
                ...
                let mut next_slot = unsafe { core::hint::assert_unchecked(len != 0); (slot + 1) % len };
                // Move everything past this element forward in the ring
                while next_slot != self.tail {
                    self.ring[slot] = self.ring[next_slot];
                    slot = next_slot;
                    next_slot = unsafe { core::hint::assert_unchecked(len != 0); (next_slot + 1) % len };
                }
                self.tail = slot;
                return Some(val);
            }
            slot = unsafe { core::hint::assert_unchecked(len != 0); (slot + 1) % len };
        }
        None
    }
        ...
        F: FnMut(&T) -> bool,
    {
        ...
        while src != self.tail {
            if f(&self.ring[src]) {
                ...
                dst = unsafe { core::hint::assert_unchecked(len != 0); (dst + 1) % len };
            }
            src = unsafe { core::hint::assert_unchecked(len != 0); (src + 1) % len };
        }
